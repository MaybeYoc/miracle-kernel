				 ...
				 gfp_t gfp)
{
	...
	gfp_t pcpu_gfp = gfp & (GFP_KERNEL | __GFP_NOWARN);
...
#define PCPU_SETUP_BUG_ON(cond)	do {					\
	...
	group_sizes = memblock_alloc(ai->nr_groups * sizeof(group_sizes[0]),
				     SMP_CACHE_BYTES);
	unit_map = memblock_alloc(nr_possible_cpu_ids * sizeof(unit_map[0]),
				  SMP_CACHE_BYTES);
	unit_off = memblock_alloc(nr_possible_cpu_ids * sizeof(unit_off[0]),
				  SMP_CACHE_BYTES);
	...
	for (group = 0, unit = 0; group < ai->nr_groups; group++, unit += i) {
		...
		for (i = 0; i < gi->nr_units; i++) {
			...
			PCPU_SETUP_BUG_ON(cpu >= nr_possible_cpu_ids);
				...
				pcpu_fc_cpu_distance_fn_t cpu_distance_fn)
{
	...
	for (upa = max_upa; upa; upa--) {
		 ...
		 */
		if (wasted > nr_possible_cpu_ids / 3)
			continue;
